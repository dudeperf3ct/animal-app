...
def select_apis(base_image, key, orig_image):
    ...
    if choice == "Wolfram API" and base_image is not None:
        add_seprator()
        st.markdown(
            ...
            if json_result["queryresult"]["success"]:
                ...
                for i in range(len(json_result["queryresult"]["pods"])):
                    ...
                    if (
                        ...
                        properties = json_result["queryresult"]["pods"][i]["subpods"][
                            0
                        ]["plaintext"]
                        for p in properties.split("\n"):
                            cols.append(p.split("|")[0].strip("\n"))
                            vals.append(p.split("|")[1].strip("\n"))
                df = pd.DataFrame(
                    {
                ...
                st.write("Basic properties of the species")
                sub_df = pd.DataFrame([vals], columns=cols)
                st.table(sub_df)
                return df
        else:
...
def main():
    ...
    st.markdown(
        "<h2 style='text-align: center; color: red;'>Animal species classification</h2>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<h4 style='text-align: center; color: red;'>Choose any animal image and get the species of the animal</h4>",
        unsafe_allow_html=True,
    )
