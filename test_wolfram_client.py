...
def main(img_path):
    ...
    if json_result["queryresult"]["success"]:
        ...
        for i in range(len(json_result["queryresult"]["pods"])):
            if json_result["queryresult"]["pods"][i]["title"] == "Scientific name":
            ...
            if "Wikipedia summary" in json_result["queryresult"]["pods"][i]["title"]:
                url = json_result["queryresult"]["pods"][i]["subpods"][0]["infos"][
                    "links"
                ]["url"]
            if (
                ...
                properties = json_result["queryresult"]["pods"][i]["subpods"][0][
                    "plaintext"
                ]
                cols, vals = [], []
                ...
                print(sub_df)
        # print(sci_names)
        # print(kingdom, phylum, kls, order, family, genus, species, url)
